...
export const getCoursesWithJoin = () => {
  ...
  INNER JOIN public.teachers t ON c.teachers_id = t.id;`; //c => abbreviation of "courses" t => abbreviation of "teachers"
  return db.query(sql).then((res) => res.rows);
};
...
export const updateTeacher = (args: IUpdateTeacher) => {
  const sql = `UPDATE public.teachers SET name = '${args.name}', age = ${args.age} WHERE id = ${args.id}`; //why WHERE id = ${args.id}?
  return db.query(sql).then((res) => res); // Why not res.rows?
};

export const deleteTeacher = (args: { id: number }) => {
  const sql = `DELETE FROM public.teachers WHERE id = ${args.id}`; //why WHERE id = ${args.id}
  return db.query(sql).then((res) => res);
};
